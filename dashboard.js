...
function chainCard(item) {
  ...
  return `<article class="chain-card" data-detail-kind="chains" data-id="${esc(item.event_chain_id)}">
    <div class="chain-head"><div><span class="chain-symbol">${icon('chain')}</span><div class="chain-topic"><small>事件链追踪主题</small><b>${esc(item.chain_name || (nodes[0]?.fact ? clip(nodes[0].fact, 66) : '事件发展脉络'))}</b><span class="chain-primary-event">一级事件 · ${esc(item.primary_event_name || '待归类')}</span></div></div><div>${chip(item.current_state)}${chip(`${item.node_count} 个节点`, 'info')}</div></div>
...
async function loadSummary() {
  ...
  $('#chain-annotations').innerHTML = [['官方确认',annotations.official_confirmation],['重大进展',annotations.major_progress],['反转',annotations.reversal],['预警',annotations.warning],['预警升级',annotations.realerts]].map(([key, value]) => `<span>${key}<b>${fmt(value)}</b></span>`).join('');
}
...
async function loadChains() {
  ...
  try {
    const data = await intelligenceQuery('chains', {page:state.page, size:8, q:$('[data-search="chains"]').value.trim(), state:'all'});
    state.more = data.has_more;
...
function renderChainDetail(record) {
  ...
  return detailFrame('事件链详情', record.chain_name || (nodes[0]?.fact ? clip(nodes[0].fact, 80) : '完整事件链'), `<div class="chain-primary-event-detail"><span>归属一级事件</span><b>${esc(record.primary_event_name || '待归类')}</b></div><div class="chain-detail-topic"><span>追踪主题</span><b>${esc(record.chain_name || '事件发展脉络')}</b></div><div class="polished-chain">${nodes.map((node, index) => `<article class="polished-node ${node.news_id ? 'is-openable' : ''}" ${node.news_id ? `data-chain-news-id="${esc(node.news_id)}" tabindex="0" role="button" aria-label="查看第 ${index + 1} 个节点对应新闻详情"` : ''}><div class="node-rail"><span>${index + 1}</span>${index < nodes.length - 1 ? '<i></i>' : ''}</div><div class="node-body"><time>${esc(node.publish_time || node.date || '时间待定')}</time><p>${esc(node.fact)}</p><div class="node-tags">${(node.annotations || []).map(tag => `<span class="${['反转','预警','预警升级'].includes(tag) ? 'alert' : ''}">${esc(tag)}</span>`).join('')}</div>${node.news_id ? '<span class="chain-news-open">点击卡片查看对应新闻详情 →</span>' : ''}</div></article>`).join('')}</div>`, 'chain-detail');
}
