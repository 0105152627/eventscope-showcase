...
const sectionGuides = {
  ...
  chains: {number:'08', title:'事件发展脉络', summary:'把同一主题下真正相关的事件按时间连接，呈现从起因到结果的发展过程。', process:['从最终事件中选择主体一致、主题一致且关系明确的节点。','按起因、推进、市场反应、处置和结果整理逻辑。','依据完整发布时间排序，形成可追溯的事件发展脉络。'], points:['重点事件链均强调节点之间的真实联系与连贯逻辑。','每条链归属一个最适合的一级事件类型。','点击节点可查看对应原始新闻，核对时间与事实。']},
};
